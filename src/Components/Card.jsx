...
export default function Card(props) {
  ...
  const getBookmarks = async () => {
    try {
      if (userId) {
        ...
        bookmark.includes(props.rec.id)
          ? setIsBookmark(true)
          : setIsBookmark(false);
      }
    } catch (err) {
  ...
  const toggleBookmark = async () => {
    try {
      setIsLoading(true);
      if (props.removeBookmark) props.removeBookmark(props.rec.id);
      if (userId) {
        const response = await axios.post(`${ServerApi}/user/bookmark`, {
          _id: userId,
