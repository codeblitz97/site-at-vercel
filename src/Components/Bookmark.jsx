    ...
    try {
      if (userId) {
        ...
        const animeDataArray = await Promise.all(
          bookmark.map(async (bookmarkItem) => {
            const animeRes = await axios.get(
              `https://animetrix-api.vercel.app/meta/anilist/info/${bookmarkItem}`
            );
        ...
        <>
          <section className="profile-wrapper">
            <div className="profile-greeting">
              {/* <h1> Hi, {user}</h1> */}
              <h1>
                <i class="fa-solid fa-bookmark continue-icon"></i> Bookmarks
              </h1>
            </div>
