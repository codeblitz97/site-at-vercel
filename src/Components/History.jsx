      ...
      if (userId) {
        ...
        const animeDataPromises = history.map(async (historyItem) => {
          const animeRes = await axios.get(
            `${HomeApi}/meta/anilist/info/${historyItem.animeId}`
          );
  ...
  const deleteHistory = async () => {
    try {
      ...
      if (conf) {
        ...
        const res = await axios.delete(`${ServerApi}/user/history/${userId}`);
        await getHistory();
        if (res && res.data) {
        ...
        <>
          <section className="profile-wrapper">
            <div className="profile-greeting">
              <h1 className="history-head">
                <i class="fa-solid fa-clock-rotate-left lastwatch-icon continue-icon"></i>{' '}
                Continue Watching
              </h1>
            </div>
                    ...
                    {animeData?.slice(0, count).map((animeDataHis, index) => {
                      return (
                        <div className="movie-card">
                          <div
                            className="lastwatch-close"
                            onClick={(ev) =>
                              removeHistory(history[index].animeId)
                            }
                          >
                          ...
                          <div className="card-head">
                            <Link
                              to={`/watch/${history[index].epId}/${history[index].animeId}`}
                            >
                              <img
                                src={animeDataHis?.image}
                                alt="its just an images"
                                className="card-img"
                              />
                            </Link>
                            <div className="card-details">
                              <h5 className="card-title">
                                {animeDataHis.title?.userPreferred ||
                                  animeDataHis.title?.english ||
                                  animeDataHis.romaji}
                              </h5>
                            </div>
                          </div>
