...
export default function Stream(props) {
  ...
  const dubStream = async () => {
    ...
    try {
      const dubVideo = await axios.get(
        `${StreamApi}/api/v2/episode/${animeId}`
      );
  ...
  const getStream = async () => {
    try {
      const Video = await axios.get(`${StreamApi}/api/v2/stream/${episodeId}`);
      if (Video.length === 0) {
        setLoading(true);
      }
    ...
    <>
      <Helmet>
        ...
        <meta
          ...
          content={`Watch or download ${episodeId} for free on Animetrix `}
        />
        <meta
          property="og:image"
            ...
            <div className="stream-container">
              <div className="video-title">
                ...
                <p>
                  ...
                  nspl player.
                  <br />
                  To play the dubbed version, change the episode to the next
                  available dub episode.
                </p>
              </div>
              <div className="playerchange-div">
                {videoDub.length > 0 && (
                  <button onClick={() => dubSwitchClick()}>
                    {dubList ? 'Sub' : 'Dub'}
                  </button>
                )}
                <i
